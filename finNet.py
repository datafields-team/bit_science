...
#filename to read
  ...
  def init_net(self, params):
      ...
      with tf.device('/gpu:0'):
          ...
          self.W_conv1 = self.weight_variable([conv_square, conv_square, channels, conv1_nodes])
          self.b_conv1 = self.bias_variable([conv1_nodes])
          self.x_image = tf.reshape(self.x, [-1,input_height, input_width, channels])
          # relu
          self.h_conv1 = tf.nn.relu(self.conv2d(self.x_image, self.W_conv1) + self.b_conv1)
          # max pool
          self.h_pool1 = self.max_pool_2x2(self.h_conv1)
          # create second convolution layer
          self.W_conv2 = self.weight_variable([conv_square, conv_square, conv1_nodes, conv2_nodes])
          self.b_conv2 = self.bias_variable([conv2_nodes])
          # relu
          self.h_conv2 = tf.nn.relu(self.conv2d(self.h_pool1, self.W_conv2) + self.b_conv2)
          #max pool
          self.h_pool2 = self.max_pool_2x2(self.h_conv2) 
          # fully connected layer
          self.W_fc1 = self.weight_variable([(input_height / 4) * (input_width / 4) * conv2_nodes, full_nodes])
          self.b_fc1 = self.bias_variable([full_nodes])
          self.h_pool2_flat = tf.reshape(self.h_pool2, [-1, (input_height / 4) * (input_width / 4) * conv2_nodes])
          #relul
          ...
          #output layer L
          self.W_fc2 = self.weight_variable([full_nodes, 1])
